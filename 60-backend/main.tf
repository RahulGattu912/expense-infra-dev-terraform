...
resource "null_resource" "expense" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo sh /tmp/backend.sh ${var.environment}" # dev
    ]
  }
}
...
resource "aws_ec2_instance_state" "backend" {
    instance_id = aws_instance.backend.id
    state = "stopped"
    depends_on = [ null_resource.backend ] # explicitly declaring dependency
}
...
# to delete the instance from local system (user system)
resource "null_resource" "backend_delete" {
    ...
    provisioner "local-exec" {
        command = "aws ec2 terminate-instances --istance-ids ${aws_instance.backend.id}"
    }
...
resource "aws_autoscaling_group" "backend" {
    ...
    timeouts {
        delete = "10m" # increased to 10m from 5m
    }
